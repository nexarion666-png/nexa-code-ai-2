...
export async function listGitHubRepos() { const client = await octokit(); const { data } = await client.rest.repos.listForAuthenticatedUser({ visibility: "all", affiliation: "owner,collaborator,organization_member", per_page: 100, sort: "updated", }); return data as GitHubRepo[]; }
