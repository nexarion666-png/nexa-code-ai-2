    ...
    <section className="flex min-h-0 flex-1 flex-col bg-panel">
      ...
      <div className="flex flex-col border-b border-line bg-[#07111e]">
        <div className="flex items-center gap-2 px-3 py-2 overflow-x-auto">
          <div className="flex items-center rounded-xl bg-panel2 p-1 shrink-0">
          ...
          <div className="flex items-center gap-2 ml-auto shrink-0">
            <button onClick={downloadZip} disabled={downloading} className="flex items-center gap-2 rounded-lg bg-white px-3 py-2 text-xs font-semibold text-black hover:bg-slate-200 disabled:opacity-60">
              <Download size={15}/> {downloading? "Zipping…" : "Download"}
            </button>
            <button onClick={() => void openPush()} disabled={pushing} className="flex items-center gap-2 rounded-lg border border-violet-400/30 bg-violet-500/20 px-3 py-2 text-xs font-semibold text-violet-100 hover:bg-violet-500/30">
              <Github size={14}/> Push to GitHub
            </button>
            <button onClick={openDeploy} className="flex items-center gap-2 rounded-lg bg-black px-3 py-2 text-xs font-semibold text-white ring-1 ring-white/10 hover:bg-slate-900">
        ...
        <div className="flex items-center gap-1 px-3 pb-2 overflow-x-auto">
          {visibleFiles.map((file) => (
            <button key={file.path} onClick={() => onOpen(file.path)} className={`flex max-w-[180px] items-center gap-2 rounded-t-xl border border-b-0 border-line px-3 py-2 text-xs shrink-0 ${file.path === current.path? "bg-[#0a1422] text-white" : "bg-panel2 text-muted hover:text-white"}`}>
      ...
      {pushOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 backdrop-blur-sm">
        <div className="w-full max-w-md rounded-2xl border border-line bg-[#0a1422] p-5 shadow-2xl">
          <div className="mb-5 flex items-center justify-between"><div><h2 className="font-semibold flex items-center gap-2"><Github size={18}/> Push to GitHub</h2><p className="mt-1 text-xs text-muted">Push all {files.length} files. Select particular repo below.</p></div><button onClick={() => setPushOpen(false)} className="rounded-lg p-2 text-muted hover:bg-white/5">×</button></div>
          {repos.length? <select value={selectedRepo} onChange={e => setSelectedRepo(e.target.value)} className="w-full rounded-xl border border-line bg-panel2 px-4 py-3 text-sm outline-none text-white">{repos.map(repo => <option key={repo.id} value={repo.id}>{repo.full_name} {repo.private? "(Private)" : "(Public)"}</option>)}</select> : <div className="rounded-xl border border-line bg-panel2 p-4 text-sm text-muted">No repositories found. Create one.</div>}
          {pushMessage && <div className="mt-3 rounded-xl border border-line bg-panel2 p-3 text-xs text-slate-300">{pushMessage}</div>}
          <div className="mt-5 flex flex-wrap justify-end gap-2"><button onClick={() => setCreateRepoOpen(true)} className="rounded-xl border border-line px-3 py-2 text-xs text-slate-300 hover:bg-white/5">Create new repo</button><Button onClick={() => void pushSelected()} variant="primary" disabled={!selectedRepo || pushing ||!files.length}>{pushing? "Pushing…" : `Push ${files.length} files`}</Button></div>
        </div>
      </div>}
